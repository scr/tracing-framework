...
goog.require('wtf.io.cff.parts.EventBufferPart');
...
wtf.io.cff.chunks.EventDataChunk = function(opt_chunkId) {
   ...
   * @type {wtf.io.cff.parts.EventBufferPart}
   * @private
   */
    ...
    switch (part.getType()) {
      ...
      case wtf.io.cff.PartType.EVENT_BUFFER:
        this.eventBufferPart_ =
            /** @type {!wtf.io.cff.parts.EventBufferPart} */ (part);
        break;
...
wtf.io.cff.chunks.EventDataChunk.prototype.init = function(
  ...
  if (needsBuffer) {
    ...
    this.eventBufferPart_ = new wtf.io.cff.parts.EventBufferPart(buffer);
  }
 ...
 * @return {!wtf.io.Buffer} Event data buffer.
 */
wtf.io.cff.chunks.EventDataChunk.prototype.getBuffer = function() {
  goog.asserts.assert(this.eventBufferPart_);
  var buffer = this.eventBufferPart_.getValue();
...
wtf.io.cff.chunks.EventDataChunk.prototype.reset = function() {
  ...
  if (this.eventBufferPart_) {
    ...
    // Reset buffer data.
    // This also resets string table data.
