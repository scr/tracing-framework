 ...
 */
wtf.trace.createTransport_ = function(options, opt_targetValue) {
  var targetValue =
  ...
  if (targetUrl == 'null') {
    ...
    throw 'Null transport not yet supported.';
    ...
    if (options.getOptionalString('wtf.trace.mode') == 'snapshotting') {
      ...
      throw 'Buffered HTTP transport not yet supported.';
    } else {
      //return new wtf.io.StreamingHttpWriteStream(targetUrl);
      throw 'Streaming HTTP transport not yet supported.';
    }
    ...
    targetValue.needsLibraryDispose = true;
    return transport;
  }
...
wtf.trace.createStreamTarget_ = function(options, transport) {
  ...
  switch (formatValue) {
    default:
    case 'binary':
      return new wtf.io.cff.BinaryStreamTarget(
          transport);
    case 'json':
...
wtf.trace.start = function(opt_options) {
  ...
  switch (options.getOptionalString('wtf.trace.mode')) {
    ...
    case 'stream':
    case 'streaming':
      // var transport = this.createTransport_(options, opt_targetValue);
...
wtf.trace.snapshot = function(opt_targetValue) {
  ...
  var transport = wtf.trace.createTransport_(options, opt_targetValue);
  ...
  goog.global.console.log(blob);
  goog.fs.blobToString(blob).addCallback(function(value) {
    goog.global.console.log(value);
    goog.global.console.log(goog.global.JSON.parse(value));
  });
};
